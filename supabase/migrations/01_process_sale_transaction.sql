  ...
  loop
    ...
    if p_status = 'completed' then
      if p_type = 'sale' then
         ...
         -- Increase stock & update cost price (simple average could be implemented here, but for now just update last cost)
         update products 
         set quantity = quantity + v_qty,
             cost_price = (v_item->>'price')::numeric -- Update cost to new purchase price
         where id = (v_item->>'product_id')::uuid;
